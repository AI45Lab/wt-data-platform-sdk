#!/usr/bin/env python3
"""
Main ETL script for ingesting data from S3 into serving table.
...
def main():
    parser = argparse.ArgumentParser(
        description='ETL data from S3 to serving table',
...
Examples:
  ...
  python scripts/existing_data_etl/run_serving_etl.py --allow-duplicates
    ...
    parser.add_argument(
        '--s3-prefix',
        required=True,
        help='S3 prefix path to process (e.g., s3://wind-tunnel-landing/owner/dataset/type/dt_v1/)'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Validate and convert data but don\'t insert into serving table'
    )
    parser.add_argument(
        '--s3-endpoint',
        default=None,
    ...
    etl = ServingDataETL(s3_endpoint=args.s3_endpoint)
